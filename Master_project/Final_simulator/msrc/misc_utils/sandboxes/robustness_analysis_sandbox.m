P = rss(10,2,3); 
K_lqry = lqry(P,eye(size(P.C,1)),eye(size(P.B,2)))

multipliers = linspace(0.3, 3, 50); 
all_poles = zeros([length(multipliers ), length(P.A)]); 
for i = 1:length(multipliers)
    poles = eig( P.A - P.B*K_lqry*multipliers(i)); 
    all_poles(i,:) = poles; 
end
