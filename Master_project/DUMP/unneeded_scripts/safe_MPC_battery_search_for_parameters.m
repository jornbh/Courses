...
if ~exist("simulink_model_name", "var")
    ...
    MPC_name_prefix = "LQR_integrated_agnostic"; 
end
...
% set_param("sandbox",'FastRestart','on')
% best_params  = simulannealbnd(@MPC_search, initial_attempt, [],[], sim_aneal_options )
% set_param("sandbox",'FastRestart','off')

%% Genetic algoritm
% ga_options = optimoptions( "ga", "PlotFcn", { "gaplotbestf", "gaplotscores"});
% set_param("sandbox",'FastRestart','on')
% best_params = ga(@MPC_search, length(initial_attempt),[],[],[],[],[],[],[], ga_options)
% set_param("sandbox",'FastRestart','off')
%% Test
...
%             hold off
            ...
            overshoot_cost = 1e-3*(norm(steam_error, Inf) + norm(O2_error, Inf) + 1e-4 * norm(air_diff_error, Inf));
            L2_norm_cost = 1e1*((norm(steam_error, 2) + norm(O2_error, 2) + 1e-4 * norm(air_diff_error, 2)) / length(steam_error));
            ...
            min_w_noise = 0.1; % Assume the noise starts at a frequency at around 0.1 (Maybe??)

            if MPC_name_prefix == "LQR_integrated"
                bode_cost = bode_based_cost(LQRDiffIntegral_params, lqe_params, full_estimated_plant, min_w_noise);
            else
                bode_cost = bode_based_cost(LQR_params, lqe_params, full_estimated_plant, min_w_noise);
            end

            cost = sim_cost + (bode_cost / (2));
        catch ME
            ME
            cost = 1e8; % A big punishment if the system is unstable
        end
