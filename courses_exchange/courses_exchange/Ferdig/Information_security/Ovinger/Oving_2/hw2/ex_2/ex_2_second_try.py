...
g_I = 2
p_S = "JAoEC1xIHAMdEEEFYA0FCw1AQxw=" #! PASSWORD 
...
async def solve():
    async with websockets.connect(TO_URL_S) as websocket:
        ...
        salt = NB2I(await websocket.recv())
        ...
        u_B =  H(  A_B + B_B   )                                #! u = H( A || B ) 
        u_I = B2I(U_B)
                
        colon_B   = (  ":" ).encode("utf-8")                  
        p_B           = (  p_S   ).encode("utf-8")      

        salt_B   = I2B(salt_I) 
        
        
        inner_hash_B = H( U_B + b":" + p_B) 
        x_B =  H( salt_B + inner_hash_B)                        #! x = H(salt || H( U || ":" || password)) 
        x_I = B2I(x_B)
        ...
        final_hash_B = H(hash_input_B)


        final_hash_NB = B2NB(final_hash_B)
