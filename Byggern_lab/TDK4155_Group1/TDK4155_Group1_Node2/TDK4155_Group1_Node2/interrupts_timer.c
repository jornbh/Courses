...
void _setTimerValue(int timestep){
    ...
    OVR3x = temp; 
    // OCR3xH = (char)(0xFF&temp); // Both H and L- registers can usually be set by OCRnx= temp
...
void _enableTimerInterrupts(){
    ...
    SET_BIT(TMSK3, OCIE3X); // X = A,B,C ( pin [3:1])
    // Enables interrupts on compare-match on compare-match A,B or C 
    CLEAR_BIT(TMSK3, TOIE3); // Bit 0
