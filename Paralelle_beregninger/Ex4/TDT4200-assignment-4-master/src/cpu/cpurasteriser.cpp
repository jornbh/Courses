...
class timers{
  public:
    std::chrono::high_resolution_clock::time_point starts[N];
    std::chrono::high_resolution_clock::duration timespans[N];
    void start(int i){
      starts[i] = std::chrono::high_resolution_clock::now();
    }
    void end(int i){
      timespans[i] += std::chrono::high_resolution_clock::now()-starts[i];
    }
    void reset(){
      for(int i =0; i<N; ++i){
        timespans[i] = std::chrono::high_resolution_clock::duration();
      }
    }
    int getTime_ms(int i){
      return std::chrono::duration_cast<std::chrono::milliseconds>(timespans[i]).count();
    }
};
...
std::vector<unsigned char> rasteriseCPU(std::string inputFile, unsigned int width, unsigned int height, unsigned int depthLimit) {
  ...
  std::cout << "Rendering an image on the CPU.." << std::endl;
    ...
    for(unsigned int item = 0; item < totalItemsToRender; item++) {
        ...
        for (unsigned int i = 0; i < meshes.size(); i++) {
            Mesh mesh = meshes.at(i); // Making them copies removed the bad printing (?)
  ...
  TIMERS.end(ALL_T);
  std::cout << "Procesing work queue  ="<< TIMERS.getTime_ms(WORK_T)<<" ms\n";
  std::cout << "Entire rasterization  ="<< TIMERS.getTime_ms(ALL_T)<<" ms\n";
	return frameBuffer;
}
