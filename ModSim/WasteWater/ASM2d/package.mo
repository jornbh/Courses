...
equation
  ...
  annotation (Documentation(info="This component models the ASM2d processes and reactions taking place in an unaerated (denitrification) tank
...
equation
  ...
  inputXmeoh = (In.Xmeoh - Xmeoh)*In.Q/V;
  inputXmep = (In.Xmep - Xmep)*In.Q/V;
  annotation (Documentation(info="This component models the ASM2d processes and reactions taking place in an aerated (nitrification) tank
of a wastewater treatment plant.
...
equation
  ...
  inputXmep = (In.Xmep - Xmep)*In.Q/V;
  annotation (Documentation(info="This ASM2d component is used to model the chemical Phosphorus precipitation
...
equation
  ...
  connect(S3.Up, S4.Dn)
    annotation (Line(points={{-2.22045e-15,-40},{-2.22045e-15,-36}}));
  connect(S5.Up, S6.Dn)
    annotation (Line(points={{-2.22045e-15,-2},{-2.22045e-15,2}}));
  connect(S6.Up, S7.Dn)
    annotation (Line(points={{-2.22045e-15,17},{-2.22045e-15,21}}));
  connect(S7.Up, S8.Dn)
  ...
  connect(S4.Up, S5.Dn)
    annotation (Line(points={{-2.22045e-15,-21},{-2.22045e-15,-17}}));
  connect(S8.Up, S9.Dn)
    annotation (Line(points={{-2.22045e-15,55},{-2.22045e-15,59}}));
  connect(Feed, S6.In)
    annotation (Line(points={{-100,10},{-67.5,10},{-67.5,9.8},{-35,9.8}}));
  connect(S1.PQw, Waste)
    annotation (Line(points={{17.5,-93},{17.5,-100},{30,-100}}));
  connect(S10.Out, Effluent)
    annotation (Line(points={{35,85.5},{67.5,85.5},{67.5,57},{100,57}}));
  connect(S1.PQr, Return)
    annotation (Line(points={{-21,-93},{-21,-100},{-30,-100}}));
  // total sludge concentration in clarifier feed
  Xf = Feed.Xtss;
  ...
  rXmep = Feed.Xmep/Xf;
  annotation (Documentation(info="This component models an ASM2d 10 - layer secondary clarifier model with 4 layers above the feed_layer (including top_layer)
...
model pump "ASM2d wastewater pump"
  ...
  Interfaces.WWFlowAsm2din In
    annotation (Placement(transformation(extent={{-110,-43},{-90,-23}})));
  Interfaces.WWFlowAsm2dout Out
    annotation (Placement(transformation(extent={{90,18},{110,38}})));
  Modelica.Blocks.Interfaces.RealInput u
    annotation (Placement(transformation(extent={{-98,15},{-78,35}})));
equation
  H = 0.5*(-Q_min + Q_max) + u*0.5*(-Q_min + Q_max) + Q_min;
  ...
  Out.Xmeoh = In.Xmeoh;
  Out.Xmep = In.Xmep;
  annotation (Documentation(info="This component models an ASM2d wastewater pump. It generates a wastewater flow
...
equation
  Out.Q = -data;
  annotation (Documentation(info="This component is used to feed an ASM2d wwtp model with flow data from measurement
when e.g. concentration is measured after the primary clarifier.

The dimension of InPort is 1.

  1 volumeflowrate Q of incoming wastewater [m3/d]"), Diagram(coordinateSystem(
        ...
        grid={2,2}), graphics={Ellipse(
            ...
            34,-44},{30,-46},{26,-48},{22,-50},{16,-52},{10,-54},{4,-54},{0,-54},
            {-8,-54}},
            lineColor={0,0,255},
            ...
            pattern=LinePattern.None,
            fillColor={0,95,191},
            fillPattern=FillPattern.Solid),Line(points={{-4,-54},{-4,-74},{88,-74}},
          thickness=0.5),Line(points={{4,-54},{4,-68},{88,-68}}, thickness=0.5)}));
end FlowSource;
...
equation
  ...
  Out.Xmeoh = data[18];
  Out.Xmep = data[19];
  annotation (Documentation(info="This component provides all ASM2d data at the influent of a wastewater treatment plant.
...
model EffluentSink "Receiving water (river)"
  ...
  annotation (Documentation(info="This component terminates an ASM2d wastewater treatment plant model e.g. the wastewater flow to the receiving water.
"));
end EffluentSink;
...
model SludgeSink "Wastesludge sink"
  ...
  annotation (Documentation(info="This component terminates the waste sludge stream of an ASM2d wastewater treatment plant model.
...
model ControlledDivider2 "Controlled flow divider"
  ...
  Interfaces.WWFlowAsm2din In
    annotation (Placement(transformation(extent={{-110,-7},{-90,13}})));
  Interfaces.WWFlowAsm2dout Out1
    annotation (Placement(transformation(extent={{90,15},{110,35}})));
  Interfaces.WWFlowAsm2dout Out2
    annotation (Placement(transformation(extent={{90,-25},{110,-5}})));
  Modelica.Blocks.Interfaces.RealInput u annotation (layer="icon", Placement(
        transformation(
        origin={0,-60},
        extent={{-10,-10},{10,10}},
        rotation=90)));
equation
  ...
  Out2.Xmeoh = In.Xmeoh;
  Out2.Xmep = In.Xmep;
  annotation (Documentation(info="This component divides one wastewater flow (ASM2d) into two flows which are controlled by the signal u (0...1).
...
model mixer2 "Mixer of two ASM2d characterised flows"
  ...
  Interfaces.WWFlowAsm2din In1
    annotation (Placement(transformation(extent={{-110,15},{-90,35}})));
  Interfaces.WWFlowAsm2din In2
    annotation (Placement(transformation(extent={{-110,-25},{-90,-5}})));
  Interfaces.WWFlowAsm2dout Out
...
model mixer3 "Mixer of 3 ASM2d characterised flows"
  ...
  Interfaces.WWFlowAsm2din In1
    annotation (Placement(transformation(extent={{-110,25},{-90,45}})));
  Interfaces.WWFlowAsm2din In2
    annotation (Placement(transformation(extent={{-110,-15},{-90,5}})));
...
model sensor_COD "Ideal sensor to measure chemical oxygen demand (COD)"
  ...
  Modelica.Blocks.Interfaces.RealOutput COD
    annotation (Placement(transformation(extent={{88,-10},{108,10}})));
equation
  In.Q = 0.0;
  COD = In.Sf + In.Sa + In.Si + In.Xi + In.Xs + In.Xh + In.Xpao + In.Xpha + In.Xa;
  annotation (Documentation(info="This component measures the chemical oxygen demand (COD) concentration [g/m3]
...
model sensor_NH "Ideal sensor to measure ammonium nitrogen"
  ...
  Interfaces.WWFlowAsm2din In
    annotation (Placement(transformation(extent={{-10,-110},{10,-90}})));
  Modelica.Blocks.Interfaces.RealOutput Snh
    annotation (Placement(transformation(extent={{88,-10},{108,10}})));
equation
  In.Q = 0;
  Snh = In.Snh;
  annotation (Documentation(info="This component measures the ammonium nitrogen concentration [g/m3]
of ASM2d wastewater and provides the result as output signal (to be
further processed with blocks of the Modelica.Blocks library).
"));
end sensor_NH;
...
model sensor_NO "Ideal sensor to measure nitrate nitrogen"
  ...
  Interfaces.WWFlowAsm2din In
    annotation (Placement(transformation(extent={{-10,-110},{10,-90}})));
  Modelica.Blocks.Interfaces.RealOutput Sno
    annotation (Placement(transformation(extent={{88,-10},{108,10}})));
equation
  In.Q = 0;
  Sno = In.Sno;
...
"), Diagram(coordinateSystem(
        ...
        grid={2,2}), graphics={Ellipse(
            ...
            fillPattern=FillPattern.Solid),Text(extent={{-80,100},{80,60}},
          textString="%name"),Line(points={{0,50},{0,38}}, thickness=0.5),Line(
          points={{-50,0},{38,0}}, thickness=0.5),Line(points={{50,0},{38,0}},
          thickness=0.5),Line(points={{-36,34},{-28,26}}, thickness=0.5),Line(
          points={{34,36},{26,28}}, thickness=0.5),Line(points={{0,0},{26,28}},
          thickness=0.5),Polygon(
            points={{30,32},{10,24},{24,12},{30,32}},
            lineColor={0,0,0},
            fillColor={0,0,0},
            fillPattern=FillPattern.Solid),Text(extent={{-36,-10},{36,-32}},
          textString="O2"),Line(points={{0,-50},{0,-90}}, thickness=0.5),Line(
          points={{50,0},{88,0}})}));
end sensor_O2;
...
model sensor_PO "Ideal sensor to measure dissolved phosphorus"
  ...
  Interfaces.WWFlowAsm2din In
    annotation (Placement(transformation(extent={{-10,-110},{10,-90}})));
  Modelica.Blocks.Interfaces.RealOutput Spo
    annotation (Placement(transformation(extent={{88,-10},{108,10}})));
equation
  In.Q = 0;
  Spo = In.Spo;
  annotation (Documentation(info="This component measures the dissolves phosphorus concentration [g/m3]
of ASM2d wastewater and provides the result as output signal (to be
further processed with blocks of the Modelica.Blocks library).
"));
end sensor_PO;
...
equation
  ...
  In.Xmeoh = Out.Xmeoh;
  In.Xmep = Out.Xmep;
  annotation (Documentation(info="This component measures the flow of an ASM2d wastewater stream and provides
...
equation
  ...
  TKN[2] = TKN[1] + In.Sno;
  annotation (Documentation(info="This component measures the Total Kjeldal Nitrogen (TKN) and the
...
model sensor_TP
  ...
  Interfaces.WWFlowAsm2din In
    annotation (Placement(transformation(extent={{-10,-110},{10,-90}})));
  Modelica.Blocks.Interfaces.RealOutput TP
    annotation (Placement(transformation(extent={{88,-10},{108,10}})));
equation
  In.Q = 0.0;
...
model sensor_TSS
  ...
  Interfaces.WWFlowAsm2din In
    annotation (Placement(transformation(extent={{-10,-110},{10,-90}})));
  Modelica.Blocks.Interfaces.RealOutput TSS
    annotation (Placement(transformation(extent={{88,-10},{108,10}})));
equation
  In.Q = 0;
  TSS = In.Xtss;
  annotation (Documentation(info="This component measures the total suspended solids concentration [g/m3]
of ASM2d wastewater and provides the result as output signal (to be
further processed with blocks of the Modelica.Blocks library).
"));
end sensor_TSS;
